---
description: "Review all uncommitted changes (staged and unstaged)"
---
...
!`mkdir -p .tmp && DIFF_FILE=".tmp/changes-diff-$(date +%Y%m%d-%H%M%S).txt" && git diff HEAD > "$DIFF_FILE" && echo "Diff saved to: $DIFF_FILE ($(wc -l < "$DIFF_FILE" | tr -d ' ') lines)"`
...
If the diff above is empty, respond with "No changes to review." and stop.
...
2. Save report to `.reviews/review-changes-{YYYY-MM-DDTHH-MM-SS}.md` with:
   - Summary (same as console output)
   - Full diff reviewed
   - Raw JSON with all comments
3. Report: `**Full review saved to**: .reviews/review-changes-{timestamp}.md`
